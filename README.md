...
| **Brevo** | Email quên mật khẩu (HTTPS API, free tier, chạy trên Render Free) |
...
- **Quên mật khẩu**: Brevo HTTPS gửi link SPA (`APP_FRONTEND_RESET_URL`); token one-time TTL 30 phút (Redis hoặc bảng `password_reset_tokens`).
...
| `MAIL_ENABLED`, `BREVO_API_KEY`, `MAIL_FROM`, `APP_FRONTEND_RESET_URL` | Quên MK (Brevo HTTPS) |
...
3. Điền secrets (JWT, Cloudinary, VNPay, Google, Brevo, …).
